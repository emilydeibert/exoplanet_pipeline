...
def process_k(k, sysr, magerr, wave, berv, Vsys, RV, Kp, phase, orders_to_correlate, model_wvl, model_conv):
    
    print(f"PID={os.getpid()} processing k={k}")
    ...
    for idx, order in enumerate(orders_to_correlate):
        ...
        F_model=interpolate.interp1d(model_wvl, model_conv, kind='linear', bounds_error=False, fill_value=np.nan) #pickling issue
        )
    ...
    return k, cmap_results, fmap_results
        ...
        for ndx, night in enumerate(nights):
            ...
            for camera in config.camera:
                ...
                with ProcessPoolExecutor(max_workers=15) as executor:
                    results = executor.map(process_k,
                    k_list,
                    repeat(sysr),
                    repeat(magerr),
                    repeat(wave),
                    ...
                    repeat(Kp),
                    repeat(phase),
                    repeat(orders_to_correlate),
                    repeat(model_wvl),
                    repeat(model_conv))
                    
                    for k, cmap_results, fmap_results in results:
                        np.savez_compressed(
